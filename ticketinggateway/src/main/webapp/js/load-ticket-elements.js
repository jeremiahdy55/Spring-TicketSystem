...
function loadTicketTableHtml(ticketData, authority) {
    ...
    // Initialize the table header for the delete column (if necessary)
    let deleteHdr = (["USER", "MANAGER"].some(role => authority.includes(role))) ? '<th>Delete</th>' : ""; 

    // Create the table header and wrapping divs
...
function changeTicketStatus(ticketId, comments, baseURL) {
    if (comments.trim() !== "") {
        comments = "?comments=" + encodeURIComponent(comments); // construct the RequestParam "comments" if needed
    };
